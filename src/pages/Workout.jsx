...
export default function Workout({ user }) {
  ...
  const addExtraExercise = () => {
    ...
    setExercises(ex => [...ex, { id, name: extraName.trim(), group: 'Dodatkowe', sets: 3, reps: '10', noWeight: false, isExtra: true }])
    setLog(l => ({...l, [id]: Array.from({length:3}, ()=>({weight:'', reps:''}))}))    
  ...
  return (
    <div>
      ...
      <div style={{marginBottom:16,padding:'12px 16px',background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:'var(--radius-sm)',display:'flex',alignItems:'center',gap:10}}>
        ...
        <label style={{fontSize:13,color:'var(--text2)',whiteSpace:'nowrap'}}>Data treningu:</label>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:12,padding:'14px 16px',cursor:'pointer'}} onClick={()=>expanded[ex.id]?setExpanded(e=>({...e,[ex.id]:false})):initSets(ex)}>
                    <div style={{flex:1}}>
                      ...
                      <div style={{display:'flex',gap:6,marginTop:4,alignItems:'center',flexWrap:'wrap'}}>
                        ...
                        {isBodyweight&&<span style={{fontSize:11,color:'var(--text3)',background:'var(--bg3)',padding:'2px 8px',borderRadius:10}}>wlasna masa</span>}
          ...
          <div style={{marginBottom:16}}>
            {showExtra ? (
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <input value={extraName} onChange={e=>setExtraName(e.target.value)} placeholder='Nazwa cwiczenia (np. Bieznia 30 min)' onKeyDown={e=>e.key==='Enter'&&addExtraExercise()} style={{flex:1}}/>
                <button onClick={addExtraExercise} className='btn-primary' style={{width:'auto',padding:'10px 16px',whiteSpace:'nowrap'}}>Dodaj</button>
                <button onClick={()=>setShowExtra(false)} className='btn-ghost' style={{padding:'10px 12px'}}>X</button>
              </div>
            ) : (
              <button onClick={()=>setShowExtra(true)} style={{display:'flex',alignItems:'center',gap:6,background:'transparent',border:'1px dashed var(--border)',color:'var(--text2)',padding:'10px 16px',borderRadius:'var(--radius-sm)',fontSize:13,width:'100%',justifyContent:'center'}}><Plus size={14}/> Dodaj wlasne cwiczenie (np. bieznia, cardio)</button>
            )}
          </div>
