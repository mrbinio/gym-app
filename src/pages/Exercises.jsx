      ...
      {showForm && (
        <div className="card" style={{ marginBottom:20, border:'1px solid var(--accent)44' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <div><label style={{ fontSize:12, color:'var(--text2)', marginBottom:6, display:'block' }}>Dzien treningowy</label>
              <select value={form.day} onChange={e=>setForm(f=>({...f,day:e.target.value}))}>
                {Object.entries(DAYS).map(([k,v])=><option key={k} value={k}>{v.label} - {v.sub}</option>)}
              </select>
            </div>
