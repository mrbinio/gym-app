    ...
    <div>
      ...
      <div className='card' style={{marginBottom:20}}>
        ...
        <select value={selected} onChange={e=>setSelected(e.target.value)}>
          {['A','B','C','D'].map(day=>{
            ...
            return exs.length?<optgroup key={day} label={'Dzien '+day}>{exs.map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</optgroup>:null
          })}
        </select>
      </div>
      {loading ? <div style={{color:'var(--text3)',textAlign:'center',padding:40}}>Ladowanie...</div> : chartData.length===0 ? (
        <div className='card' style={{textAlign:'center',padding:'40px 20px',color:'var(--text3)'}}>
          Brak danych dla tego cwiczenia.<br/>Zaloguj trening! {String.fromCharCode(128170)}
        </div>
      ) : (
        <>
