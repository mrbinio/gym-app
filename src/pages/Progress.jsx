...
export default function Progress({ user }) {
  ...
  const DAYS_T = { A:t.dayA, B:t.dayB, C:t.dayC, D:t.dayD }
  ...
  const Tip = ({active,payload,label}) => active&&payload?.length?(
    <div style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:8,padding:'10px 14px',fontSize:13}}>
      ...
      {payload.map(p=><div key={p.name} style={{color:p.color}}>{p.name==='maxWeight'?t.maxWeight:p.name==='volume'?t.volume:t.reps}: <strong>{p.value} kg</strong></div>)}
    </div>
  ):null

  return (
    <div>
      <h1 style={{fontFamily:'var(--font-display)',fontSize:32,letterSpacing:2,marginBottom:4}}>{t.progressTitle}</h1>
      <p style={{color:'var(--text3)',fontSize:13,marginBottom:24}}>{t.progressSub}</p>
      <div className="card" style={{marginBottom:20}}>
        <label style={{fontSize:12,color:'var(--text2)',marginBottom:8,display:'block'}}>{t.selectExercise}</label>
        <select value={selected} onChange={e=>setSelected(e.target.value)}>
          {['A','B','C','D'].map(day=>{const exs=allEx.filter(e=>e.day===day);return exs.length?<optgroup key={day} label={DAYS_T[day]}>{exs.map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</optgroup>:null})}
        </select>
      </div>
      {loading?<div style={{color:'var(--text3)',textAlign:'center',padding:40}}>{t.loading}</div>:chartData.length===0?(
        <div className="card" style={{textAlign:'center',padding:'40px 20px',color:'var(--text3)'}}>{t.noData} &#128170;</div>
      ):(
        <>
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:20}}>
            {[{label:t.record,value:best+' kg',color:'var(--accent)'},{label:t.latest,value:(latest?.maxWeight||0)+' kg',color:'var(--text)'},{label:t.change,value:(trend>=0?'+':'')+trend+' kg',color:trend>=0?'var(--success)':'var(--danger)'}].map(({label,value,color})=>(
          ...
          <div className="card" style={{marginBottom:16}}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:16}}><TrendingUp size={16} color="var(--accent)"/><span style={{fontSize:14,fontWeight:600}}>{t.maxWeight}</span></div>
          ...
          <div className="card" style={{marginBottom:16}}>
            <div style={{fontSize:14,fontWeight:600,marginBottom:16}}>{t.volume}</div>
          ...
          <div className="card">
            <div style={{fontSize:14,fontWeight:600,marginBottom:14}}>{t.history}</div>
            {[...chartData].reverse().map((d,i)=>(
              <div key={i} style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,padding:'10px 0',borderBottom:'1px solid var(--border)',fontSize:13}}>
                <span style={{color:'var(--text2)'}}>{d.date}</span><span style={{color:'var(--accent)',fontWeight:600}}>{d.maxWeight} kg</span><span style={{color:'var(--text3)'}}>{d.totalReps} {t.reps}</span>
              </div>
            ))}
          </div>
        </>
