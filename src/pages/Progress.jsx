...
export default function Progress({ user }) {
  ...
  useEffect(() => {
    ...
    workouts.forEach(w => {
      ...
      if (ex?.sets?.length) {
        ...
        data.push({ date:w.date?.toDate().toLocaleDateString('pl-PL',{day:'numeric',month:'short'}), maxWeight:maxW, totalReps:totalR, volume:Math.round(vol) })
      }
    })
    setChartData(data)
  }, [selected, workouts])
  ...
  const Tip = ({ active, payload, label }) => active&&payload?.length ? (
    <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:8, padding:'10px 14px', fontSize:13 }}>
      ...
      {payload.map(p=><div key={p.name} style={{ color:p.color }}>{p.name==='maxWeight'?'Max ciezar':p.name==='volume'?'Objetosc':'Powt.'}: <strong>{p.value}{p.name==='maxWeight'?' kg':p.name==='volume'?' kg':''}</strong></div>)}
    </div>
  ) : null
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom:20 }}>
        ...
        <select value={selected} onChange={e=>setSelected(e.target.value)}>
          {['A','B','C','D'].map(day => { const exs=allEx.filter(e=>e.day===day); return exs.length?<optgroup key={day} label={'Dzien '+day}>{exs.map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</optgroup>:null })}
        </select>
      </div>
      {loading ? <div style={{ color:'var(--text3)', textAlign:'center', padding:40 }}>Ladowanie...</div> : chartData.length===0 ? (
        <div className="card" style={{ textAlign:'center', padding:'40px 20px', color:'var(--text3)' }}>Brak danych dla tego cwiczenia.<br/>Zaloguj trening! &#128170;</div>
      ) : (
        <>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:20 }}>
            {[{ label:'Rekord', value:best+' kg', color:'var(--accent)' },{ label:'Ostatnio', value:(latest?.maxWeight||0)+' kg', color:'var(--text)' },{ label:'Zmiana', value:(trend>=0?'+':'')+trend+' kg', color:trend>=0?'var(--success)':'var(--danger)' }].map(({label,value,color})=>(
          ...
          <div className="card" style={{ marginBottom:16 }}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="#1e2d45" /><XAxis dataKey="date" tick={{fontSize:11,fill:'#475569'}} /><YAxis tick={{fontSize:11,fill:'#475569'}} /><Tooltip content={<Tip/>}/><Line type="monotone" dataKey="maxWeight" stroke="var(--accent)" strokeWidth={2} dot={{fill:'var(--accent)',r:4}} /></LineChart>
            </ResponsiveContainer>
          </div>
          <div className="card" style={{ marginBottom:16 }}>
            <div style={{ fontSize:14, fontWeight:600, marginBottom:16 }}>Objetosc (kg x powt.)</div>
            <ResponsiveContainer width="100%" height={160}>
              <LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="#1e2d45" /><XAxis dataKey="date" tick={{fontSize:11,fill:'#475569'}} /><YAxis tick={{fontSize:11,fill:'#475569'}} /><Tooltip content={<Tip/>}/><Line type="monotone" dataKey="volume" stroke="#3b82f6" strokeWidth={2} dot={{fill:'#3b82f6',r:3}} /></LineChart>
            </ResponsiveContainer>
          </div>
          <div className="card">
            ...
            {[...chartData].reverse().map((d,i)=>(
              <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, padding:'10px 0', borderBottom:'1px solid var(--border)', fontSize:13 }}>
                <span style={{ color:'var(--text2)' }}>{d.date}</span><span style={{ color:'var(--accent)', fontWeight:600 }}>{d.maxWeight} kg</span><span style={{ color:'var(--text3)' }}>{d.totalReps} powt.</span>
              </div>
            ))}
          </div>
        </>
