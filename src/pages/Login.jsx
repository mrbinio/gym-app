...
export default function Login() {
  ...
  const handle = async (e) => {
    ...
    } catch(e) { setErr(t('login.errors.'+e.code) || e.message) }
    setLoading(false)
  }
  const googleLogin = async () => {
    setErr(''); setLoading(true)
    try { await signInWithPopup(auth,googleProvider) } catch { setErr(t('login.errors.google')) }
    setLoading(false)
  }
    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:20, background:'var(--bg)' }}>
      <div style={{ width:'100%', maxWidth:380 }}>
        <div style={{ position:'absolute', top:16, right:16 }}>
          <button onClick={toggle} style={{ background:'var(--bg3)', border:'1px solid var(--border)', color:'var(--text2)', padding:'5px 12px', borderRadius:20, fontSize:12, fontWeight:600, cursor:'pointer' }}>
            {lang==='pl'?'EN':'PL'}
          </button>
        </div>
        <div style={{ textAlign:'center', marginBottom:40 }}>
          ...
          <div style={{ marginTop:8, color:'var(--text3)', fontSize:13 }}>{t('login.tagline')}</div>
        </div>
        <div className="card" style={{ padding:28 }}>
          <h2 style={{ fontSize:18, fontWeight:600, marginBottom:24 }}>{mode==='login'?t('login.title'):t('login.register')}</h2>
          <form onSubmit={handle} style={{ display:'flex', flexDirection:'column', gap:14 }}>
            <div><label style={{ fontSize:12, color:'var(--text2)', marginBottom:6, display:'block' }}>{t('login.email')}</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="email@example.com" required/></div>
            <div><label style={{ fontSize:12, color:'var(--text2)', marginBottom:6, display:'block' }}>{t('login.password')}</label><input type="password" value={pass} onChange={e=>setPass(e.target.value)} placeholder="••••••••" required/></div>
            ...
            <button type="submit" className="btn-primary" disabled={loading}>{loading?t('loading'):mode==='login'?t('login.loginBtn'):t('login.registerBtn')}</button>
          </form>
