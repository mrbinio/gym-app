  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(140px, 1fr))', gap:12, marginBottom:28 }}>
        {[{ label:'Wszystkie treningi', value:stats.total, icon:Dumbbell, color:'#3b82f6' },{ label:'Ten tydzien', value:stats.thisWeek, icon:Flame, color:'var(--accent)' },{ label:'Cwiczenia w planie', value:21, icon:TrendingUp, color:'#22c55e' }].map(({ label, value, icon:Icon, color }) => (
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
            {recent.map(w => (
              <div key={w.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 0', borderBottom:'1px solid var(--border)' }}>
                ...
                <div style={{ flex:1 }}><div style={{ fontSize:14, fontWeight:500 }}>{DAYS[w.day]?.sub||w.day}</div><div style={{ fontSize:12, color:'var(--text3)' }}>{w.date?.toDate().toLocaleDateString('pl-PL')}</div></div>
